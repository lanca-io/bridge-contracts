...
export async function upgradeProxyImplementation(hre, proxyType: ProxyEnum, shouldPause: boolean) {
    const chainName = hre.network.name as CNetworkNames
    const { viemChain } = conceroNetworks[chainName]

    const implementationKey = shouldPause ? envPrefixes.pause : envPrefixes[ProxyEnum.lancaBridge]
